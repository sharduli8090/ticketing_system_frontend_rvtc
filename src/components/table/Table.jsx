        ...
        <table className="max-w-full rounded-lg shadow-lg divide-y divide-blue-300 m-0">
          <thead className="bg-blue-500 text-white">
            <tr>
              ...
              {keys.map((key, index) => (
                <th key={index} className="px-6 py-3 text-center">{key}</th>
              ))}
          ...
          <tbody className="text-gray-600">
            {data.map((row, index) => (
              <tr key={index} className={index % 2 === 0 ? 'bg-blue-100' : ''}>
                ...
                {keys.map((key, idx) => (
                  <td key={idx} className="px-6 py-4 text-ellipsis overflow-hidden whitespace-nowrap text-center">
                    {row[key]}
                  </td>
                ))}
                <td className="px-6 py-4 text-center">
                  <button onClick={() => openUpdateModal(row)} className="bg-blue-500 hover:bg-blue-700 rounded mr-3 text-white text-sm p-2">Update</button>
                  <button onClick={() => handleDeleteRow(row.id, 'employee')} className="bg-red-500 hover:bg-red-700 rounded mr-3 text-white text-sm p-2">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="flex items-center justify-center min-h-screen">
            ...
            <div className="relative bg-white rounded-lg p-8">
              ...
              <form onSubmit={submitForm}>
                <div>
                  <label htmlFor="inputField" className="block mb-2">Input Field:</label>
                  <input id="inputField" type="text" className="w-full border rounded px-3 py-2" value={inputValue} onChange={(e) => setInputValue(e.target.value)} required />
                </div>
